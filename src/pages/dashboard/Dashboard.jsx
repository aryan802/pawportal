	...
	<div className="min-h-[80vh] flex flex-col items-center justify-center bg-gradient-to-br from-pink-50 to-blue-50 py-10">
		<div className="bg-white rounded-3xl shadow-xl p-8 max-w-2xl w-full flex flex-col items-center relative">
			...
			<div className="flex gap-4 mb-8">
				<Link to="/login">
					<button className="bg-pink-500 shadow text-white px-6 py-2 rounded-full font-semibold text-lg hover:bg-pink-600 transition">Login</button>
				</Link>
				<Link to="/register">
					<button className="bg-white border-2 border-pink-400 text-pink-500 px-6 py-2 rounded-full font-semibold text-lg hover:bg-pink-50 transition">Register</button>
				</Link>
			</div>
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full mt-2">
				<Link to="/pet-listings">
					<button className="w-full bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl font-medium shadow hover:bg-blue-100 transition">Browse Pet Listings</button>
				</Link>
				<Link to="/community-forums">
					<button className="w-full bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-xl font-medium shadow hover:bg-yellow-100 transition">Community Forums</button>
				</Link>
				<Link to="/lost-found">
					<button className="w-full bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl font-medium shadow hover:bg-green-100 transition">Lost &amp; Found</button>
				</Link>
				<Link to="/events">
					<button className="w-full bg-purple-50 border border-purple-200 text-purple-700 px-4 py-3 rounded-xl font-medium shadow hover:bg-purple-100 transition">Events</button>
				</Link>
			</div>
		</div>
	</div>
